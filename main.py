...
def projects():
    ...
    if 'loggedin' in session:
        ...
        if form.is_submitted():
            result = request.form
            cursor.execute('SELECT * FROM projects WHERE project_name = %s', [result['pname']])
            # for row in cursor:
            #     print(row)
...
def donate():
    ...
    if 'loggedin' in session:
        if request.method == "POST":
            ...
            phone = details['phone']
            cur = mysql.connection.cursor()
            cur.execute("INSERT INTO donorinfo VALUES (NULL, %s, %s, %s, %s, %s, %s,%s)", (name,pname,amount,email,address,role,phone))
            mysql.connection.commit()
            cur.close()
